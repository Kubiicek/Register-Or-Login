...
import controller.JLoginCryptography;
...
public class JLogin extends JFrame {
	...
	public JLogin() {
		...
		JButton btnLog = new JButton("Continue");
		btnLog.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String username = textFieldUsuario.getText();
                char[] password = passwordField.getPassword();

                if (JLoginCryptography.authenticateUser(username, password)) {
                    JOptionPane.showMessageDialog(btnLog, "Login successful.");
                } else {
                    JOptionPane.showMessageDialog(btnLog, "Invalid username or password.", "Warning",
                            JOptionPane.WARNING_MESSAGE);
                }
			}
		});
		btnLog.setBackground(new Color(49, 62, 64));
		btnLog.setForeground(Color.WHITE);
		btnLog.setBounds(149, 191, 89, 23);
		panel.add(btnLog);
